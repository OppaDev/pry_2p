...
@section('content')
    <div class="w-full px-6 py-6 mx-auto text-lg">
        <div class="container-fluid py-4">
            <h1 class="mb-0 text-2xl font-semibold text-slate-700">LISTA DE ASIGNATURAS</h1>
        </div>
                    ...
                    <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                        <div class="flex justify-between items-center">
                            <h6 class="mb-0 text-xl font-semibold text-slate-700">
                                <i class="fas fa-book mr-2 text-slate-700"></i>
                                ASIGNATURAS
                            </h6>
                            <div class="flex items-center space-x-3">
                                <!-- Formulario de búsqueda -->
                                <form method="GET" action="{{ route('asignaturas.index') }}" class="flex items-center space-x-2">
                                    ...
                                    <div
                                        ...
                                        @if ($search)
                                            <a href="{{ route('asignaturas.index', ['per_page' => $perPage]) }}"
                        ...
                        @if ($search)
                            <div
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-2 text-sm text-blue-600">({{ $asignaturas->total() }}
                                        {{ $asignaturas->total() == 1 ? 'resultado' : 'resultados' }})</span>
                                </div>
                            </div>
                        @endif
                    </div>
                                            ...
                                            <td
                                                ...
                                                <div class="flex px-2 py-1">
                                                    <div class="flex flex-col justify-center">
                                                        <h6 class="mb-0 text-lg leading-normal">{{ $asignatura->nombre }}</h6>
                                                    </div>
                                                </div>
                                            </td>
                                            <td
                                                class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <p class="mb-0 text-lg font-semibold leading-tight">{{ $asignatura->codigo }}</p>
                                            </td>
                                            <td
                                                class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <span
                                                    class="text-lg font-semibold leading-tight text-slate-400">{{ $asignatura->created_at->format('d/m/y') }}</span>
                                            </td>
                                            <td
                                                class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <span
                                                    class="text-lg font-semibold leading-tight text-slate-400">{{ $asignatura->updated_at->format('d/m/y') }}</span>
                                            </td>
                                            <td
                                                class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <div class="flex items-center space-x-2 action-buttons">
                                                    ...
                                                    <button type="button" onclick="openModal('delete-asignatura-{{ $asignatura->id }}-modal')"
                                        ...
                                        <tr>
                                            <td colspan="5"
                                                ...
                                                <div class="flex flex-col items-center py-8">
                                                    ...
                                                    <p class="text-xl font-medium text-slate-500">No hay asignaturas disponibles</p>
                                                    <p class="text-lg text-slate-400">
                                                        @if($search)
                                                            No se encontraron asignaturas que coincidan con tu búsqueda.
                                                        @else
                                                            Aún no has agregado ninguna asignatura.
                                                        @endif
                                                    </p>
                                                </div>
                                            </td>
    ...
    <!-- Modales de Confirmación de Eliminación - Solo cargar cuando hay asignaturas -->
    @if($asignaturas->count() > 0)
        @foreach($asignaturas as $asignatura)
            @include('components.delete-modal', [
                'modalId' => 'delete-asignatura-' . $asignatura->id . '-modal',
                'title' => 'Confirmar Eliminación de Asignatura',
                'message' => '¿Estás seguro de que deseas eliminar esta asignatura? Esta acción lo enviará a la papelera y podrá ser restaurado posteriormente.',
                'itemName' => $asignatura->nombre,
                'itemDetails' => 'Código: ' . $asignatura->codigo,
                'deleteRoute' => route('asignaturas.destroy', $asignatura->id),
