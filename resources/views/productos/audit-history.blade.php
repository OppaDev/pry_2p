...
@section('content')
    <div class="w-full px-6 py-6 mx-auto text-lg">
        ...
        <div class="flex flex-wrap -mx-3">
            <!-- Información del Producto (pequeña) -->
            <div class="w-full px-3 mb-4">
                <div class="flex items-center p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200/60">
                    ...
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-slate-700">{{ $producto->nombre }}</h3>
                        <div class="flex items-center space-x-4 text-sm text-slate-600">
                            <span><i class="fas fa-barcode mr-1"></i>{{ $producto->codigo }}</span>
                            <span><i class="fas fa-cubes mr-1"></i>{{ $producto->cantidad }} unidades</span>
                            <span><i class="fas fa-dollar-sign mr-1"></i>${{ number_format($producto->precio, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center space-x-3">
                                <!-- Filtros -->
                                <form method="GET" action="{{ route('productos.audit-history', $producto->id) }}" class="flex items-center space-x-2">
                        ...
                        @if($eventFilter)
                            <div class="relative w-full p-3 text-indigo-700 bg-indigo-100 border border-indigo-300 rounded-lg mt-4">
                                <div class="flex items-center">
                                    ...
                                    <a href="{{ route('productos.audit-history', $producto->id) }}" class="ml-2 text-indigo-600 hover:text-indigo-800">
                                        ...
                                        <div class="relative p-6 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
                                                ...
                                                <div class="flex items-center space-x-3">
                                                    @switch($audit->event)
                                                        @case('created')
                                                            ...
                                                            <div>
                                                                <h4 class="text-lg font-semibold text-slate-700">Producto Creado</h4>
                                                                <p class="text-sm text-slate-500">Se registró un nuevo producto</p>
                                                            </div>
                                                            @break
                                                        @case('updated')
                                                            ...
                                                            <div>
                                                                <h4 class="text-lg font-semibold text-slate-700">Producto Actualizado</h4>
                                                                <p class="text-sm text-slate-500">Se modificaron los datos del producto</p>
                                                            </div>
                                                            @break
                                                        @case('deleted')
                                                            ...
                                                            <div>
                                                                <h4 class="text-lg font-semibold text-slate-700">Producto Eliminado</h4>
                                                                <p class="text-sm text-slate-500">El producto fue eliminado (soft delete)</p>
                                                            </div>
                                                            @break
                                                        @case('restored')
                                                            ...
                                                            <div>
                                                                <h4 class="text-lg font-semibold text-slate-700">Producto Restaurado</h4>
                                                                <p class="text-sm text-slate-500">El producto fue restaurado desde papelera</p>
                                                            </div>
                                                            @break
                                                    @endswitch
                                                </div>
                                            ...
                                            @if($audit->old_values || $audit->new_values)
                                                <div class="border-t border-slate-100 pt-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div class="space-y-2">
                                                                    @foreach($audit->old_values as $key => $value)
                                                                        <div class="flex justify-between text-sm p-2 bg-red-50 rounded-lg">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div class="space-y-2">
                                                                    @foreach($audit->new_values as $key => $value)
                                                                        <div class="flex justify-between text-sm p-2 bg-green-50 rounded-lg">
                                                                            <span class="font-medium text-slate-700">
                                                                                @switch($key)
                                                                                    @case('nombre')
                                                                                        <i class="fas fa-tag mr-1"></i>Nombre:
                                                    ...
                                                    <!-- Resumen de cambios para productos -->
                                                    @if($audit->event == 'updated' && $audit->old_values && $audit->new_values)
                                                        <div class="mt-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200/50">
                                                            <h6 class="text-sm font-semibold text-blue-700 mb-2">
                                                                ...
                                                                @foreach($audit->new_values as $key => $newValue)
                                                                    @if(isset($audit->old_values[$key]) && $audit->old_values[$key] != $newValue)
                                                                        <div class="text-center p-2 bg-white/60 rounded">
                                                                            <div class="font-medium text-slate-600">{{ ucfirst($key) }}</div>
                                                                            @if($key == 'precio')
                                                                                <div class="text-red-600">${{ number_format($audit->old_values[$key], 2) }}</div>
                                                                                <div class="text-xs text-slate-400">↓</div>
                                                                                <div class="text-green-600">${{ number_format($newValue, 2) }}</div>
                                                                                @php
                                                                                    $difference = $newValue - $audit->old_values[$key];
                                                                                    $percentage = $audit->old_values[$key] > 0 ? ($difference / $audit->old_values[$key]) * 100 : 0;
                                                                                @endphp
                                                                                <div class="text-xs {{ $difference >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                                                                    {{ $difference >= 0 ? '+' : '' }}{{ number_format($percentage, 1) }}%
                                                                                </div>
                                                                            @elseif($key == 'cantidad')
                                                                                <div class="text-red-600">{{ $audit->old_values[$key] }}</div>
                                                                                <div class="text-xs text-slate-400">↓</div>
                                                                            ...
                                                                            @else
                                                                                <div class="text-red-600 truncate">{{ Str::limit($audit->old_values[$key], 8) }}</div>
                                                                                <div class="text-xs text-slate-400">↓</div>
                                                                                <div class="text-green-600 truncate">{{ Str::limit($newValue, 8) }}</div>
                                                                            @endif
                                                                        </div>
                                                                    @endif
                                                                @endforeach
                            ...
                            @else
                                <div class="flex flex-col items-center py-12">
                                    ...
                                    <p class="text-slate-500 text-center">
                                        @if($eventFilter)
                                            No se encontraron eventos de tipo "{{ ucfirst($eventFilter) }}" para este producto.
                                        @else
                                            Este producto aún no tiene historial de cambios registrado.
                                        @endif
                                    </p>
                                </div>
                            @endif
